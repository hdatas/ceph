...
#ifndef HMEM_DB_STORE_H
#define HMEM_DB_STORE_H
...
enum {
  l_hmemdb_first = 34300,
  l_hmemdb_gets,
  l_hmemdb_txns,
  l_hmemdb_compact,
  l_hmemdb_compact_range,
  l_hmemdb_compact_queue_merge,
  l_hmemdb_compact_queue_len,
  l_hmemdb_last,
};

/**
 * Uses HmemDB to implement the KeyValueDB interface
 */
class HmemDBStore : public KeyValueDB {
  CephContext *cct;
  PerfCounters *logger;
  string path;
  ...
  class CompactThread : public Thread {
    HmemDBStore *db;
  public:
    CompactThread(HmemDBStore *d) : db(d) {}
    void *entry() {
      db->compact_thread_entry();
      return NULL;
    }
    friend class HmemDBStore;
  } compact_thread;
...
public:
   ...
   * on initialization and then passed through to HmemDB.
   * We transform a couple of these into actual HmemDB
   * structures, but the rest are simply passed through unchanged. See
   * leveldb/options.h for more precise details on each.
   *
   * Set them after constructing the HmemDBStore, but before calling
   * open() or create_and_open().
   */
  struct options_t {
    uint64_t write_buffer_size; /// in-memory write buffer size
    int max_open_files; /// maximum number of files HmemDB can open at once
  ...
  HmemDBStore(CephContext *c, const string &path) :
    cct(c),
    logger(NULL),
    path(path),
    db_cache(NULL),
    ...
    filterpolicy(NULL),
#endif
    compact_queue_lock("HmemDBStore::compact_thread_lock"),
  ...
  struct HmemDBOp {
    ...
    std::string key;
    bufferlist data;
    HmemDBOp(HmemOpType type, const string &key) : type(type), key(key) {}
    HmemDBOp(HmemOpType type, const string &key, const bufferlist &data) : type(type), key(key), data(data) {}
  };

  class HmemDBTransactionImpl : public KeyValueDB::TransactionImpl {
  public:
    ...
    HmemDBTransactionImpl(HmemDBStore *db) : db(db) {}
    void set(
      const string &prefix,
      const string &k,
      const bufferlist &bl);
  ...
  KeyValueDB::Transaction get_transaction() {
    return ceph::shared_ptr< HmemDBTransactionImpl >(
      new HmemDBTransactionImpl(this));
  }
  ...
  class HmemDBWholeSpaceIteratorImpl :
    public KeyValueDB::WholeSpaceIteratorImpl {
  protected:
    boost::scoped_ptr<leveldb::Iterator> dbiter;
  public:
    HmemDBWholeSpaceIteratorImpl(leveldb::Iterator *iter) :
      dbiter(iter) { }
    virtual ~HmemDBWholeSpaceIteratorImpl() { }
  ...
  class HmemDBSnapshotIteratorImpl : public HmemDBWholeSpaceIteratorImpl {
    leveldb::DB *db;
    const leveldb::Snapshot *snapshot;
  public:
    HmemDBSnapshotIteratorImpl(leveldb::DB *db, const leveldb::Snapshot *s,
				leveldb::Iterator *iter) :
      HmemDBWholeSpaceIteratorImpl(iter), db(db), snapshot(s) { }

    ~HmemDBSnapshotIteratorImpl() {
...
protected:
  WholeSpaceIterator _get_iterator() {
    return ceph::shared_ptr<KeyValueDB::WholeSpaceIteratorImpl>(
      new HmemDBWholeSpaceIteratorImpl(
  ...
  WholeSpaceIterator _get_snapshot_iterator() {
    ...
    return ceph::shared_ptr<KeyValueDB::WholeSpaceIteratorImpl>(
      new HmemDBSnapshotIteratorImpl(db.get(), snapshot,
